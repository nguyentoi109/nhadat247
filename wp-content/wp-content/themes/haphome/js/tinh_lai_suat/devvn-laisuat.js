...
(function($){
    $(document).ready(function(){
        function devvn_firstLoad() {
            var hinhthucVal = $('#hinh_thuc_vay').val();
            if(hinhthucVal != null){
                $('.devvn_section_input.active').removeClass('active');
                $('.devvn_section_input.for_'+hinhthucVal).addClass('active');
            }
            $('#result_money_vay,#result_money_gocthang,#result_money_laithang,#result_money_all').val('');
        }
        devvn_firstLoad();
        $('.devvn_icon_money').maskMoney({
            precision: 0,
            thousands:'.',
            decimal:','
        });
        $('#hinh_thuc_vay').change(function () {
            devvn_firstLoad();
        });
        function moneyToNumber(money){
            var b1 = money.match(/[0-9]+/g);
            a1 = '';
        ...
        function devvn_calc_money(tien,time,nhom,baohiem) {
            var result_money_vay = tien;
            if(baohiem == 1){
                result_money_vay = tien + ((tien*5.5)/100);
            }
            var gocthang = result_money_vay/time;
            var laithang = (result_money_vay*nhom)/100;
            var allmonth = laithang + gocthang;

            $('.devvn_ls_result').slideDown();

            $('#result_money_vay').maskMoney('mask',result_money_vay);
            $('#result_money_gocthang').maskMoney('mask',Math.ceil(gocthang));
            $('#result_money_laithang').maskMoney('mask',Math.ceil(laithang));
            $('#result_money_all').maskMoney('mask',Math.ceil(allmonth));
        }
        $.validator.addMethod('custom_money', function (value, element) {
            if(moneyToNumber(value) < 10000000 || moneyToNumber(value) > 70000000) {
                return this.optional(element) || false;
            }
            return this.optional(element) || true;
        }, "Nhập đúng số tiền cần vay");
        $.validator.addMethod('custom_money2', function (value, element) {
            if( $('#so_dien').val() == '2.95' && (moneyToNumber(value) < 10000000 || moneyToNumber(value) > 14000000) ) {
                return this.optional(element) || false;
            }else if( $('#so_dien').val() == '2.17' && (moneyToNumber(value) < 15000000 || moneyToNumber(value) > 30000000) ){
                return this.optional(element) || false;
            }else if( $('#so_dien').val() == '1.66' && (moneyToNumber(value) < 15000000 || moneyToNumber(value) > 50000000) ){
                return this.optional(element) || false;
            }
            return this.optional(element) || true;
        }, "Nhập đúng số tiền cần vay");
        $.validator.addMethod('custom_money3', function (value, element) {
            if( $('#gia_xe').val() == '2.17' && (moneyToNumber(value) < 20000000 || moneyToNumber(value) > 30000000) ) {
                return this.optional(element) || false;
            }else if( $('#gia_xe').val() == '3.38' && (moneyToNumber(value) < 12000000 || moneyToNumber(value) > 20000000) ){
                return this.optional(element) || false;
            }
            return this.optional(element) || true;
        }, "Nhập đúng số tiền cần vay");
        $.validator.addMethod('custom_money4', function (value, element) {
            if( $('#luong_nhom_ct').val() == '2.95' && (moneyToNumber(value) < 10000000 || moneyToNumber(value) > 20000000) ) {
                return this.optional(element) || false;
            }else if( ($('#luong_nhom_ct').val() == '2.17' || $('#luong_so_tien_vay').val() == '1.66') && (moneyToNumber(value) < 10000000 || moneyToNumber(value) > 70000000) ){
                return this.optional(element) || false;
            }
            return this.optional(element) || true;
        }, "Nhập đúng số tiền cần vay");
        $.validator.addMethod('custom_money5', function (value, element) {
            if( $('#phi_bao_hiem').val() == '2.17' && (moneyToNumber(value) < 10000000 || moneyToNumber(value) > 25000000) ){
                return this.optional(element) || false;
            }else if( $('#phi_bao_hiem').val() == '1.66' && (moneyToNumber(value) < 10000000 || moneyToNumber(value) > 70000000) ){
                return this.optional(element) || false;
            }
            return this.optional(element) || true;
        }, "Nhập đúng số tiền cần vay");

        $('#gia_xe').change(function(){
            var thisVal = $(this).val();
            $('#xe_so_tien_vay,#xe_time_vay').val('');
            if(thisVal == '2.17'){
                $('.for_xe .devvn_tooltip').html('Tối thiểu 20 triệu và tối đa 30 triệu đồng.');
                $('#xe_time_vay option').each(function () {
                   if($(this).val()){
                       $(this).prop('disabled',false);
                   }
                });
            }else{
                $('.for_xe .devvn_tooltip').html('Tối thiểu 12 triệu và tối đa 20 triệu đồng.');
                $('#xe_time_vay option').each(function () {
        ...
        $('#luong_nhom_ct').change(function(){
            var thisVal = $(this).val();
            $('#luong_so_tien_vay,#luong_time_vay').val('');
            if(thisVal == '2.95'){
                $('.for_luong .devvn_tooltip').html('Tối thiểu 10 triệu và tối đa 20 triệu đồng.');
                $('#luong_time_vay option').each(function () {
                    if($(this).val() == '30' || $(this).val() == '36'){
                        $(this).prop('disabled',true);
                    }
                });
            }else{
                $('.for_luong .devvn_tooltip').html('Tối thiểu 10 triệu và tối đa 70 triệu đồng.');
                $('#luong_time_vay option').each(function () {
                    if($(this).val()){
                        $(this).prop('disabled',false);
                    }
        ...
        $('#so_dien').change(function(){
            var thisVal = $(this).val();
            $('#dien_so_tien_vay').val('');
            if(thisVal == '2.95'){
                $('.for_dien .devvn_tooltip').html('Tối thiểu 10 triệu và tối đa 14 triệu đồng.');
            }else if(thisVal == '2.17'){
                $('.for_dien .devvn_tooltip').html('Tối thiểu 15 triệu và tối đa 30 triệu đồng.');
            }else{
                $('.for_dien .devvn_tooltip').html('Tối thiểu 15 triệu và tối đa 50 triệu đồng.');
            }
        });
        $('#phi_bao_hiem').change(function(){
            var thisVal = $(this).val();
            $('#bao_hiem_so_tien_vay').val('');
            if(thisVal == '2.17'){
                $('.for_baohiem .devvn_tooltip').html('Tối thiểu 10 triệu và tối đa 25 triệu đồng.');
            }else{
                $('.for_baohiem .devvn_tooltip').html('Tối thiểu 10 triệu và tối đa 70 triệu đồng.');
            }
        });
        $('#devvn_formls').on('submit',function () {
            $('.devvn_form_error_label').slideUp();
        });
        $("#devvn_formls").validate({
            rules: {
                hinh_thuc_vay: "required",

                luong_so_tien_vay: {
                    required: true,
                    custom_money4: true
                },
                luong_time_vay: "required",
                luong_nhom_ct: "required",
                luong_bao_hiem_vay: "required",

                dien_so_tien_vay: {
                    required: true,
                    custom_money2: true
                },
                dien_time_vay: "required",
                so_dien: "required",
                dien_bao_hiem_vay: "required",

                xe_so_tien_vay: {
                    required: true,
                    custom_money3: true
                },
                xe_time_vay: "required",
                gia_xe: "required",
                xe_bao_hiem_vay: "required",

                bao_hiem_so_tien_vay: {
                    required: true,
                    custom_money5: true
                },
                bao_hiem_time_vay: "required",
                phi_bao_hiem: "required",
                bh_bao_hiem_vay: "required",

                tindung_so_tien_vay: {
                    required: true,
                    custom_money: true
                },
                tindung_time_vay: "required",
                phi_tindung: "required",
                tindung_bao_hiem_vay: "required",
            },
            submitHandler: function() {
                ...
                if(hinhthucVal != null){
                    switch (hinhthucVal){
                        case 'luong':
                            var tien = moneyToNumber($('#luong_so_tien_vay').val());
                            var time = parseInt($('#luong_time_vay').val());
                            var nhom = parseFloat($('#luong_nhom_ct').val());
                            var baohiem = parseInt($('#luong_bao_hiem_vay').val());
                            devvn_calc_money(tien,time,nhom,baohiem);
                            break;
                        case 'dien':
                            var tien = moneyToNumber($('#dien_so_tien_vay').val());
                            var time = parseInt($('#dien_time_vay').val());
                            var nhom = parseFloat($('#so_dien').val());
                            var baohiem = parseInt($('#dien_bao_hiem_vay').val());
                            devvn_calc_money(tien,time,nhom,baohiem);
                            break;
                        case 'xe':
                            var tien = moneyToNumber($('#xe_so_tien_vay').val());
                            var time = parseInt($('#xe_time_vay').val());
                            var nhom = parseFloat($('#gia_xe').val());
                            var baohiem = parseInt($('#xe_bao_hiem_vay').val());
                            devvn_calc_money(tien,time,nhom,baohiem);
                            break;
                        case 'baohiem':
                            var tien = moneyToNumber($('#bao_hiem_so_tien_vay').val());
                            var time = parseInt($('#bao_hiem_time_vay').val());
                            var nhom = parseFloat($('#phi_bao_hiem').val());
                            var baohiem = parseInt($('#bh_bao_hiem_vay').val());
                            devvn_calc_money(tien,time,nhom,baohiem);
                            break;
                        case 'tindung':
                            var tien = moneyToNumber($('#tindung_so_tien_vay').val());
                            var time = parseInt($('#tindung_time_vay').val());
                            var nhom = parseFloat($('#phi_tindung').val());
                            var baohiem = parseInt($('#tindung_bao_hiem_vay').val());
                            devvn_calc_money(tien,time,nhom,baohiem);
                            break;
                        default:
                            break;
            ...
            invalidHandler: function(form, validator) {
                ...
                $('.devvn_form_error_label').slideDown();
